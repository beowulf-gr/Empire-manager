...
def end_turn(request, realm_name):
    ...
    for action in ongoing_actions:
        if action.is_completed(next_season, next_year):
            action.completed = True
            action.save()
            # Apply the effects of the completed action
            if action.action_type == "construct_farm":
                development_actions.finish_farm_construction(realm, action.data)
            elif action.action_type == "train_units":
                military_actions.finish_unit_training(realm, action.data)
            elif action.action_type == "mine_resources":
                economic_actions.apply_mining_yield(realm, action.data)
