...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for size in military_sizes:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"Created Military Unit Size: {military_unit_size.name}"))
            else:
                self.stdout.write(self.style.WARNING(f"Military Unit Size {military_unit_size.name} already exists"))