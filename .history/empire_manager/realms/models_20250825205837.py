...
class Resource(models.Model):
    ...
    category = models.CharField(max_length=50, blank=True, null=True, help_text="e.g., Minerals, Lumber")
...
class GoodsType(models.Model):
    ...
    # Cost definition
    cost_in_gold = models.DecimalField(max_digits=10, decimal_places=2, default=0.00,
...
class OngoingAction(models.Model):
    ...
    assigned_population = models.ManyToManyField(
        'PopulationUnit', 
        blank=True,
        related_name='assigned_action'
    )
...
class StrongholdType(models.Model):
    ...
    production_capacity = models.PositiveIntegerField(default=1, help_text="Number of population units that can be assigned to production actions per season.")
...
class ActionType(models.Model):
    ...
    # Link to Seasons
    available_seasons = models.ManyToManyField(Season, blank=True)
    ...
    # Define any universal costs or prerequisites here if needed.
    # For now, we'll let the logic functions handle costs since they are complex.
