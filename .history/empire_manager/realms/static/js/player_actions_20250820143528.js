    ...
    actionButtons.forEach(button => {
        button.addEventListener('click', async function(event) {
            ...
            if (actionKey === 'construct_stronghold' || actionKey === 'build_roads' || actionKey === 'build_mine' || actionKey === 'upgrade_stronghold') {
                ...
                // 1. Create the complete HTML structure for the form at once.
                dynamicInputsDiv.innerHTML = `
                    <div id="form-inputs-container">
                ...
                // This single function will handle the final calculation.
                const calculateFinalStep = async () => {
                    const goodId = goodSelect.value;
                    const quantity = document.getElementById('quantity')?.value || 1;
                    const resourceId = document.getElementById('resource_to_use')?.value;
                    if (!goodId || !quantity) return;
                ...
                strongholdSelect.addEventListener('change', async function() {
                    ...
                    if (!selectedStronghold) { displayCostPreview({}); return; }
                    ...
                    quantitySelect.addEventListener('change', calculateFinalStep);
                    ...
                    if (details.required_resource_category) {
                        ...
                        resourceSelect.addEventListener('change', calculateFinalStep);
                    }
                    
                    // ### THIS IS THE ONLY CHANGE ###
                    // Instead of calling the function directly, trigger the change event on the new dropdown.
