            ...
            if (actionKey === 'construct_stronghold' || actionKey === 'build_roads' || actionKey === 'build_mine' || actionKey === 'upgrade_stronghold') {
                ...
                dynamicInputsDiv.innerHTML = `
                    <div id="form-inputs-container">
                        ...
                        <div id="resource-choice-container" style="display: none;"></div>
                        <div id="population-dropdown-container"></div>
                    </div>
                    <div id="cost-preview-container" style="margin-top: 15px; padding: 10px; border: 1px solid #eee;"></div>
                `;
                ...
                await fetchSelectOptions(allProducibleGoodsUrl, goodSelect);

                // This single function will handle all updates to the form's state
                const updateProductionForm = async () => {
                    ...
                    // Hide elements that depend on selections
                    ...
                    // Show and populate the resource dropdown if needed
                    if (details.required_resource_category) {
                        // Only rebuild if it's not already there or has changed
                        if (resourceContainer.style.display === 'none') {
                            resourceContainer.innerHTML = `
                                <label for="resource_to_use">4. Select Resource to Use:</label>
                            ...
                            resourceSelect.addEventListener('change', updateProductionForm); // Re-attach listener
                        }
                        resourceContainer.style.display = 'block';
                    } else {
                        resourceContainer.innerHTML = ''; // Clear it if not needed
                    }

                    // Fetch the final cost from the backend
                    const response = await fetch(previewUrl, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                    ...
                    displayCostPreview(costs);
                    if (data.is_overpaying) {
                        const previewContainer = document.getElementById('cost-preview-container');
                        previewContainer.innerHTML += `<p style="color: orange; font-weight: bold;">Warning: Using this valuable resource will result in some waste.</p>`;
                    }
                ...
                // Listener for the first dropdown (Goods)
                ...
                // Listener for the second dropdown (Strongholds)
                strongholdSelect.addEventListener('change', function() {
                    ...
                    quantitySelect.addEventListener('change', updateProductionForm);
                    updateProductionForm(); // Initial call to calculate costs for quantity 1
                });
            } else {
