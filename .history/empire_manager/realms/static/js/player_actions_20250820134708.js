        ...
        button.addEventListener('click', async function(event) {
            ...
            if (actionKey === 'construct_stronghold' || actionKey === 'build_roads' || actionKey === 'build_mine' || actionKey === 'upgrade_stronghold') {
                ...
                const updateProductionForm = async () => {
                    ...
                    if (!goodId) return; // Exit if no good is selected

                    const detailsRes = await fetch(`/realm/get_good_details_json/${goodId}/`);
                    const details = await detailsRes.json();

                    // Show or hide the resource dropdown
                    if (details.required_resource_category) {
                        if (resourceContainer.style.display === 'none') {
                            ...
                            await fetchSelectOptions(`/realm/${realmName}/get_resources_by_category_json/${details.required_resource_category}/`, document.getElementById('resource_to_use'));
                            document.getElementById('resource_to_use').addEventListener('change', updateProductionForm);
                        }
                ...
                strongholdSelect.addEventListener('change', function() {
                    ...
                    updateProductionForm(); // Initial call
                });
            } else {
