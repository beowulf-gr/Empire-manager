...
def start_recruit_population(realm: Realm, post_data):
    ...
    try:
        ...
        if num_recruits > 0:
            message = f"Successfully recruited {num_recruits} {target_race_obj.name}s!"
            return True, message, None # Indicate success with message
        else:
            message = f"Recruitment attempt for {target_race_obj.name}s yielded no new population (Charisma: {charisma_modifier})."
