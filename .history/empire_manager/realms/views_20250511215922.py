...
def realm_create_automatic(request):
    if request.method == 'POST':
        # 1. Generate a random realm name
        ...
        # 2. Set the ruler
        ruler_name = request.user.username

        # 3. Generate random starting treasury
        starting_treasury = random.randint(0, 1000)

        # 4. Generate random starting resources
        starting_resources = {
            "food": random.randint(50, 300),
            "wood": random.randint(20, 150),
            "stone": random.randint(0, 100),
            "gold": random.randint(0, 50),
        }

        # 5. Create the new realm object
        ...
        # --- Start generating Land Units ---
        available_land_unit_types = LandUnitType.objects.all()  # Get all available land unit types
        num_starting_land_units = random.randint(3, 7)  # Decide on a random number of starting land units

        for i in range(num_starting_land_units):
            if available_land_unit_types:
                land_unit_type = random.choice(available_land_unit_types)
                new_land_unit = LandUnit.objects.create(
                    name=f"{land_unit_type.name} #{i+1}",
                    unit_type=land_unit_type,
                    realm=new_realm,
                )
                # Optionally assign a random mineral type if the land unit can produce minerals
                if new_land_unit.unit_type.production.get("minerals"):
                    ...
                    for mineral, chance in MINERAL_SUBTYPES:
                        ...
                        if roll <= total:
                            ...
                            break
                    if mineral_type:
                        new_land_unit.mineral_type = mineral_type
                        new_land_unit.save()

        # --- Start generating Population Units ---
        available_races = PopulationRace.objects.all()
        num_starting_population = random.randint(5, 15)
        ...
        # --- Assign initial population to land units (basic approach) ---
        unassigned_population = list(PopulationUnit.objects.filter(realm=new_realm, assigned_to=None))
        land_units = list(LandUnit.objects.filter(realm=new_realm))

        random.shuffle(unassigned_population)
        random.shuffle(land_units)

        land_unit_index = 0
        for pop_unit in unassigned_population:
            if land_unit_index < len(land_units) and len(land_units[land_unit_index].assigned_population) < land_units[land_unit_index].unit_type.settlement_capacity:
                land_unit = land_units[land_unit_index]
                land_unit.assigned_population.append({"race": pop_unit.race.name}) # Store race name for simplicity
                land_unit.save()
                pop_unit.assigned_to = land_unit
                pop_unit.save()
                # Move to the next land unit if the current one is full (very basic distribution)
                if len(land_units[land_unit_index].assigned_population) >= land_units[land_unit_index].unit_type.settlement_capacity:
                    land_unit_index += 1
            else:
                break # Stop assigning if no more land units or all are full

        # 7. Redirect to the new realm's detail view
