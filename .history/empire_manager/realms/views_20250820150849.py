...
def preview_action_cost(request, realm_name):
    if request.method == 'POST':
        ...
        if action_key == 'build_roads':
            ...
            if good_type_id:
                print(good_type_id, resource_id, quantity)  # Debugging output
...
def get_strongholds_for_good_json(request, realm_name, good_type_id):
    ...
    data = [{'id': sh.id, 'name': f"{sh.name} ({sh.land_unit.name})", 'production_capacity': sh.stronghold_type.production_capacity} for sh in eligible_strongholds]
