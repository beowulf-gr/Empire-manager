...
def edit_population(request, realm_name=None):
    PopulationUnitFormSet = modelformset_factory(
        PopulationUnit,
        form=PopulationUnitForm,
        extra=0,  # No extra forms added by default
    ...
    if realm_name:
        ...
        formset = PopulationUnitFormSet(
            queryset=PopulationUnit.objects.none(),  # No database objects for new realms
            initial=population_units_data  # Use initial data from the session
        )
                ...
                for form in formset:
                    if form.cleaned_data:
                        if form.cleaned_data.get('DELETE', False):
                            form.instance.delete()  # Delete if checkbox is checked
                        else:
                            obj = form.save(commit=False)
