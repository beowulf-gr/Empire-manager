...
def create_realm_review(request):
    ...
    return render(request, 'realms/steps/review.html', {"realm": data, "realm_name": realm_name})
...
def edit_resources(request, realm_name=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if realm_name:
                realm.resources = form.cleaned_data['resources']
                realm.save()
            else:
        ...
        form = ResourcesForm(initial={'resources': initial_value})
