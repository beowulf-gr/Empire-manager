...
def create_realm_review(request):
    ...
    if realm:
        ...
        realm_scale_name = 'N/A'
        if scale_id_from_session:
            try:
                # Fetch the RealmScale object just for display purposes
                display_scale = RealmScale.objects.get(id=scale_id_from_session)
                realm_scale_name = display_scale.name
            except RealmScale.DoesNotExist:
        ...
        context["realm"] = { # Reconstruct realm dict for template consistency
            ...
            'scale': realm_scale_name, # Add scale name here for display
