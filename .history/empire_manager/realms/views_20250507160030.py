...
def edit_land(request, realm_name=None):
    ...
    LandUnitFormSet = modelformset_factory(LandUnit, fields=('name', 'unit_type', 'assigned_population', 'upgrades', 'mineral_type'), extra=0)
...
def edit_population(request, realm_name=None):
    ...
    if request.method == 'POST':
        ...
        # Populate the initial data correctly for editing
        initial_data = []
        for unit in population_units:
            initial_data.append(unit.__dict__)  # Here, we need to clean the dict for form compatibility
            
        formset = PopulationUnitFormSet(initial=initial_data)
