...
def get_production_choices(request):
    # Get the unit_type_id from the GET request
    unit_type_id = request.GET.get('unit_type_id')

    # Make sure a unit_type_id is provided and is valid
    if not unit_type_id:
        return JsonResponse({'choices': []}, status=400)  # Return empty choices if no unit_type_id is provided

    # Get the LandUnitType object or return 404 if not found
    unit_type = get_object_or_404(LandUnitType, pk=unit_type_id)

    # Ensure choices are in the correct format (check if it's a list of dictionaries, etc.)
    if not hasattr(unit_type, 'choices') or not isinstance(unit_type.choices, list):
        return JsonResponse({'choices': []}, status=400)  # Return empty choices if 'choices' is not a valid list

    # Prepare the choices for the production field
    options = []
    for choice in unit_type.choices:
        # Assuming `choice` is a dictionary like {'key': 'value'}
        if isinstance(choice, dict):
            key = list(choice.keys())[0]
            value = list(choice.values())[0]
            options.append((str(key), f"{key}: {value}"))
        else:
            # Handle case where the choice is not a dictionary or doesn't have the expected structure
            options.append(('Invalid', 'Invalid choice format'))

    # Return the choices as JSON
    return JsonResponse({'choices': options})
...
def create_realm_step_3(request):
    if request.method == "POST":
        ...
        wood = int(request.POST.get("wood", 0))
        stone = int(request.POST.get("stone",0))
...
#     if request.method == "POST":
#         name = request.POST.get("land_name")
...
#             return redirect('create_realm_step_5')  # move on
#         else:
#             return redirect('create_realm_step_4')  # add another

#     return render(request, 'realms/steps/step_4_land.html')
...
def create_realm_step_5(request):
    ...
    return render(request, 'realms/steps/step_5_population.html')
    ...
    if request.method == "POST":
        ...
        for land in data.get('land_units', []):
            LandUnit.objects.create(
                ...
                settlement_capacity=land['settlement_capacity']
            )
...
def edit_treasury(request, realm_name=None):
    ...
    if request.method == 'POST':
        ...
        form = TreasuryForm(initial={'treasury': realm_data.get('treasury', 0)})
...
def edit_resources(request, realm_name=None):
    ...
    if request.method == 'POST':
        ...
        form = ResourcesForm(initial=realm_data.get('resources', {}))
...
def edit_land(request, realm_name=None):
    ...
    LandUnitFormSet = modelformset_factory(LandUnit, fields=('name', 'unit_type', 'production', 'choices', 'harvest', 'settlement_capacity', 'assigned_population', 'upgrades', 'mineral_type'), extra=0)
    
    if request.method == 'POST':
        formset = LandUnitFormSet(request.POST)
        if formset.is_valid():
            if realm_name:
                ...
                for form in formset:
                    if form.cleaned_data:
                        ...
                        land_unit, created = LandUnit.objects.update_or_create(
                            id=land_unit_data.get('id', None),
                            defaults=land_unit_data
                        )
            else:
        ...
        #formset = LandUnitFormSet(initial=[land_unit.__dict__ for land_unit in land_units])
...
def edit_population(request, realm_name=None):
    if realm_name:
        ...
        realm_data = request.session.get('new_realm', {})
        population_units = realm_data.get('population_units', [])

    PopulationUnitFormSet = formset_factory(PopulationUnitForm, extra=0)

    if request.method == 'POST':
        formset = PopulationUnitFormSet(request.POST)
        if formset.is_valid():
            if realm_name:
                ...
                for form in formset:
                    if form.cleaned_data:
                        ...
                        population_unit, created = PopulationUnit.objects.update_or_create(
                            id=population_unit_data.get('id', None),
                            defaults=population_unit_data
                        )
            else:
        ...
        formset = PopulationUnitFormSet(initial=[unit.__dict__ for unit in population_units])
